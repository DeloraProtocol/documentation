...
For EVM execution, the current widget implementation requires a raw EIP-1193 provider so it can:

- read the active account with `eth_accounts`
- switch chains with `wallet_switchEthereumChain`
- add missing chains with `wallet_addEthereumChain`
- submit transactions with `eth_sendTransaction`

Provide one of these for externally managed EVM execution:

- `getEvmProvider()`
- `evmProvider`

If you also expose `evmBrowserProvider` or `getEvmBrowserProvider()`, treat them as optional helpers, not as a replacement for the raw EIP-1193 provider.
...
When you provide `walletOptions`, make sure each option's `availableNamespaces` accurately reflects the ecosystems that wallet can actually connect to.
