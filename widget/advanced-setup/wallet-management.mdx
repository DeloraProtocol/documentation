...
description: "Internal and host-managed wallet connection patterns for TradeWidget."
"og:image": "https://docs.delora.build/images/OG.png"
---

The widget can either manage wallets on its own or consume wallet state from the host application.
...
The widget supports a separate destination wallet. When no separate destination wallet is connected, it can also reuse the origin wallet address when the buy-side namespace matches. Users can additionally paste a receiver address directly in the UI.
...
For the full list of exported wallet-management types, see [Widget API Reference](/widget/advanced-setup/widget-api-reference).
