...
If you pass an `origin` or `destination` object, that side is externally managed. This is presence-based, not connection-based. A side with `namespace: null` and `address: null` is still externally managed if the object exists.

Omit a side entirely when you want that side to keep the built-in wallet flow.
...
For externally managed EVM wallets, provide at least one of:
...
- tries EIP-5792 batching for approve plus swap when the wallet supports it
