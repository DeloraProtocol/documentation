...
This page summarizes the supported third-party integration API for `@deloraprotocol/widget`.
...
| --- | --- | --- |
| `apiUrl` | `string` | Delora API base. You can point this to your own backend proxy. |
...
For browser-rendered widgets, do not pass `apiKey` directly from the client. Keep it on your backend and proxy `/v1/chains`, `/v1/tokens`, `/v1/tools`, and `/v1/quotes` through `config.apiUrl`.
