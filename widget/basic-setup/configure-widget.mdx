...
Use `config.rpcUrls` to give the widget one or more preferred RPC endpoints for a numeric chain ID. List URLs in priority order.
...
<TradeWidget
  config={{
    rpcUrls: {
      ...
      // Solana mainnet
      1000000001: ["https://your-solana-rpc.example"],
    },
  }}
/>
```

`rpcUrls` has type `Partial<Record<number, string[]>>`. The widget removes blank entries and duplicates, gives your URLs priority in the supplied order when they are available, and retains built-in and Delora metadata endpoints as fallbacks. This setting only customizes chains already available in widget metadata; it does not add a new chain.

Because the widget calls these endpoints from the browser, make sure each RPC endpoint accepts cross-origin browser requests.
