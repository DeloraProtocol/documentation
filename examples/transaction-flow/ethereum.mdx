...
async function main() {
  ...
  if (!callData) {
    throw new Error("calldata is missing in the quote response");
  }

  const tx = await wallet.sendTransaction(callData);
...
Once you have a callData compatible with ethers.js, execution is simple:
