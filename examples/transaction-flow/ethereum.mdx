...
`/v1/quotes` currently uses exact-input semantics by default, so `tradeType` is intentionally not passed in this example. `senderAddress` is required, while `receiverAddress` is optional for same-type routes like this one and defaults to `senderAddress`.
