...
The Delora MCP Server exposes the following tools. Each tool maps to a corresponding [Delora API](/api-reference/alyra-api-v1) endpoint (`https://api.delora.build/v1`).
...
The response includes `inputAmount`, `outputAmount`, `adapter`, `calldata` (transaction data for the wallet), and optional `fees` and `gas` details.
