...
description: Rate limits are tracked by client IP and endpoint scope. A valid x-api-key selects the authenticated limit tier.
...
Rate limits are tracked by client IP and endpoint scope. Quote-related endpoints share one quote counter, while the rest of the public endpoints share a separate counter. A valid `x-api-key` selects the authenticated limit tier and may use a custom requests-per-minute limit configured for your integration.
...
<Note>
The Delora API does not require an API key by default. API keys are used for higher rate limits.
</Note>
